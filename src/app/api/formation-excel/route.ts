...
import { sendExcelFormationTelegram } from "@/lib/telegram";
import { appendExcelLeadToSheet } from "@/lib/google-sheets";
...
export async function POST(request: Request) {
  try {
    ...
    const [telegramResult] = await Promise.all([
      sendExcelFormationTelegram(leadData),
      appendExcelLeadToSheet(leadData).catch((error) => {
        console.error(
