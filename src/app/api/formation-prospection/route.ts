...
import { sendProspectionFormationTelegram } from "@/lib/telegram";
import { appendProspectionLeadToSheet } from "@/lib/google-sheets";
...
export async function POST(request: Request) {
  try {
    ...
    const [telegramResult] = await Promise.all([
      sendProspectionFormationTelegram(leadData),
      appendProspectionLeadToSheet(leadData).catch((error) => {
        console.error(
